<div class="box-body">
    <table id="example2" class="nobaar-table">
        ...
        <tbody>
        @foreach($moving_done_orders as $key=>$order)
            <tr>
                <td class="item_id"><?= $order->seen == 0 ? '<span class="label label-danger "> &#9734;</span>' : ''  ?>
                    <a href="{{ isset($order->id) ? route('orders.show', $order->id) : '#'}}">{{$order->id}}</a></td>
                ...
                <td class="heavyItm">
                    @foreach($order->heavyThings as $heavyThing)
                        @if ($heavyThing->pivot->count)
                            ...
                            <span
                                    class="label label-success">{{($heavyThing->pivot->count > 1) ? $heavyThing->pivot->count : ''}}</span>
                        </span>
                        @endif
                    @endforeach
                </td>
                    ...
                    <form action="{{ route('sourceAssign') }}" method="post">
                        ...
                        <select name="source" id="" onchange="this.form.submit();">
                            ...
                            @foreach($thirdparty as $val)
                                <option
                                        {{ $order->thirdparty_id== $val->id ? 'selected': '' }} value="{{$val->id}}">{{$val->user->first_name ?? '' .' '. $val->user->last_name ?? ' '}}</option>
                            @endforeach
                        </select>
                    </form>
                </td>
                <td>
                    <a href="tel://{{ $order->user->phone  != '09338931751' ?$order->user->phone: $order->receiver_phone }}">{{ $order->user->phone  != '09338931751' ?$order->user->phone: $order->receiver_phone }}</a>
                </td>
                <td>{{$order->platform}} </td>
                <td>{{dateTojal($order->moving_time ?? '') . ' '. dayOweek($order->moving_time ?? '')}} </td>
                <td>
                    <form action="{{ route('transactionType') }}" method="post"
                          id="transactionTypeForm{{ $order->id }}">
                        @csrf
                        {{ method_field('put')  }}
                        <input type="hidden" name="id" value="{{ $order->id }}">
                        <select name="type" style="width: 132px;" id="transaction_type{{ $order->id }}"
                                onchange="this.form.submit()">
                            <option value="0" {{ ($order->transaction_type ?? '') == null ? 'selected' : '' }}></option>
                            <option value="1" {{ ($order->transaction_type ?? '') == 1 ? 'selected' : '' }}>نقدی به
                                راننده
                            </option>
                            <option value="2" {{ ($order->transaction_type ?? '') == 2 ? 'selected' : '' }}>نقدی به
                                نوبار
                            </option>
                            <option value="3" {{ ($order->transaction_type ?? '') == 3 ? 'selected' : '' }}>پرداخت به
                                طرف سوم
                            </option>
                            <option value="4" {{ ($order->transaction_type ?? '') == 4 ? 'selected' : '' }}>اعتباری به
                                نوبار
                            </option>
                        </select>
                    </form>
                        ...
                        <div class="modal-dialog">
                            ...
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <h4 class="modal-title">{{ $order->user['first_name'] ?? '' }} {{$order->user['last_name'] ?? '' }}</h4>
                                </div>
