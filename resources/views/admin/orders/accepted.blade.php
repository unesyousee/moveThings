    ...
    <table id="example2" class="nobaar-table">
        ...
        @foreach($accepted_orders as $key=>$order)
            <tr>
                ...
                <td>
                    <a href="tel://{{ ($order->user->phone ?? '') == '09338931751' ? $order->receiver_phone : ($order->user->phone ?? '')}}">
                        {{  ($order->user->phone ?? '') == '09338931751' ? $order->receiver_phone : ($order->user->phone ?? '')}}
                    </a>
                </td>
                <td>{{dateTojal($order->moving_time ?? '') . ' '. dayOweek($order->moving_time ?? '')}} </td>
                <td>

                        <form action="{{ route('order.match' , $order->id) }}" method="post">
                            @csrf
                            <select name="match" id="match{{$order->match}}" onchange="this.form.submit();">
                                <option value="0" {{ $order->match == 0 ? 'selected' : ''}}>نامشخص</option>
                                <option value="1" {{ $order->match == 1 ? 'selected' : ''}}>تطبیق</option>
                                <option value="2" {{ $order->match == 2 ? 'selected' : ''}}>عدم تطبیق</option>
                            </select>
                        </form>
                </td>
                <td>
                ...
                <td>
                    <form action="{{ route('change_driver') }}" method="post">
                        ...
                        <select class="form-control" name="driver" id="" onchange="this.form.submit()">
                            ...
                            @if($driver->carrier->name == $order->carrier->name)
